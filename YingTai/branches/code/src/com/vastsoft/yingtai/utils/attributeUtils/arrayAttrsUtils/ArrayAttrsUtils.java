package com.vastsoft.yingtai.utils.attributeUtils.arrayAttrsUtils;
...
public class ArrayAttrsUtils {
		...
		if (listAttrNames!=null&&listAttrNames.size()>0&&listObject!=null&&listObject.size()>0) {
			for (Object obj : listObject) {
				...
				for (Method method : methods) {
					...
					if (IgnoreAttrHandle.needIgnore(method))continue;
					AttrEntry ae=this.attrNameContains(attributeName);
					if (ae==null)continue;
					try {
						Object to = method.invoke(obj);
						String applyAttrName=ae.newName==null||ae.getNewName().trim().isEmpty()?ae.getAttrName():ae.getNewName();
	...
	private void addExtraAttr(List listObj) {
		if (listObject==null)listObject=new ArrayList<Object>();
