...
public class Ver2RemoteDataHandler extends AbstractRemoteDataHandler {
	...
	protected void mergeEntity(AbstractShareEntity oldEntity, ShareRemoteEntityAgent rea, VsSqlSession session)
		...
		if (SharePatientDataType.PATIENT_DA.equals(pdType)) {
			...
			oldCaseHistory.replaceWith(caseHistory);
			...
			oldDicomImg.replaceWith(dicomImg);
			...
			TDicomImg dicomImg = mapper.selectDicomImgByIdForUpdate(((TSeries) oldEntity).getDicom_img_id());
			if (dicomImg == null)
				throw new RemotePatientInfoException("父影像未找到！");
			try {
				series.buildPart_type_id(dicomImg.getDevice_type_id());
			} catch (DicomImgException e) {
				e.printStackTrace();
				throw new RemotePatientInfoException(e);
			}
			oldEntity.replaceWith(series);
			mapper.updateSeries((TSeries) oldEntity);
		} else if (SharePatientDataType.PATIENT_ORG_MAPPER_DA.equals(pdType)) {
			TPatientOrgMapper patient_org_mapper = (TPatientOrgMapper) rea.getEntity();
			TPatientOrgMapper old_patient_org_mapper = (TPatientOrgMapper) oldEntity;
			PatientMapper mapper = session.getMapper(PatientMapper.class);
			old_patient_org_mapper.replaceWith(patient_org_mapper);
			mapper.updatePatientOrgMapper(old_patient_org_mapper);
		} else {
	...
	protected AbstractShareEntity tryTakeOldEntity(ShareRemoteEntityAgent rea, VsSqlSession session)
		...
		if (SharePatientDataType.PATIENT_DA.equals(pdType)) {
			...
			// if (!StringTools.isEmpty(patient.getMobile())) {
			// patient.setIdentity_id(patient.getIdentity_id().trim());
			// TPatient oldPatient =
			...
			TCaseHistory caseHistory = (TCaseHistory) rea.getEntity();
			SearchCaseHistoryParam shsp = new SearchCaseHistoryParam();
			shsp.setCase_his_num(caseHistory.getCase_his_num());
			shsp.setOrg_id(super.getCurrOrgId());
			CaseHistoryType type = CaseHistoryType.parseCode(caseHistory.getType());
			if (type == null || type.equals(CaseHistoryType.YUANZHEN))
				throw new RemotePatientInfoException("无效的病例类型！");
			shsp.setType(type);
			if (type.equals(CaseHistoryType.OTHER))
				shsp.setSource_type(caseHistory.getSourceType());
			List<FCaseHistory> listCaseHistory;
			try {
				listCaseHistory = CaseHistoryService.instance.searchCaseHistory(shsp, session);
			} catch (Exception e) {
				e.printStackTrace();
				throw new RemotePatientInfoException(e);
			}
			return CollectionTools.isEmpty(listCaseHistory) ? null : listCaseHistory.get(0);
		} else if (SharePatientDataType.DICOM_IMG.equals(pdType)) {
			...
			dicomImg.setMark_char(dicomImg.getMark_char().trim());
			dicomImg.setCase_id(oldCaseHistory.getId());
			dicomImg.setAffix_id(super.getAffix_id());//不作为查询条件
			DicomMapper mapper = session.getMapper(DicomMapper.class);
			TDicomImg oldDicomImg = mapper.selectDicomImgByMarkCharAndCaseIdForUpdate(dicomImg);
			if (oldDicomImg != null)
				return oldDicomImg;
		} else if (SharePatientDataType.DICOM_IMG_SERIES.equals(pdType)) {
			TDicomImg oldDicom = (TDicomImg) this.tryTakeOldEntity(this.getReaByUid(rea.getParentUid()), session);
			if (oldDicom == null) 
				return null;
			TSeries series = (TSeries) rea.getEntity();
			...
			DicomMapper mapper = session.getMapper(DicomMapper.class);
			TSeries oldSeries = mapper.selectSeriesByMarkCharAndImgIdForUpdate(new TSeries(oldDicom.getId(), series.getMark_char()));
			return oldSeries;
...
//			}
			// return
			// this.tryTakeOldEntity(super.getReaByUid(rea.getParentUid()),
			// session);
		} else if (SharePatientDataType.PATIENT_ORG_MAPPER_DA.equals(pdType)) {
	...
	protected void saveReaForAdd(ShareRemoteEntityAgent rea, AbstractShareEntity parentEntity, VsSqlSession session)
		...
		if (SharePatientDataType.PATIENT_DA.equals(pdType)) {
			...
			if (PatientDataSourceType.parseCode(caseHistory.getSource_type()) == null)
				throw new RemotePatientInfoException("保存病例的病例来源类型必须指定");
			...
			TDicomImg dicomImg = (TDicomImg) parentEntity;
			...
			patientOrgMapper.setPatient_id(((TPatient) parentEntity).getId());
