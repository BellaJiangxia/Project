...
.config(function($stateProvider, $urlRouterProvider){
	...
	.state("template.public",{
		url : "/public",
		templateUrl : "template/template.public.html",
		controller : "templatePublicCtrl"
	})
	.state("template.new",{
		url : "/new",
	...
	$urlRouterProvider.when("/template", "/template/public");
})
.controller("templateCtrl",function($scope,$http,$msgDialog,$window){
})
.controller("templatePublicCtrl",function($scope,$http,$msgDialog,$selectDialog,$queryService){
	$scope.listTemplate=[];
	$scope.searchOpt={
		deviceTypeId:"0",
		partTypeId:"0",
	...
	$scope.initData=function(){
		...
		$scope.searchPublicTemplate();
	};
	$scope.searchPublicTemplate=function(){
		$http.post("sys!searchPublicTemplate.action",$scope.searchOpt)
...
.controller("templateNewCtrl",function($scope,$http,$msgDialog,$queryService){
	...
	$scope.saveTemplate=function(){
		...
		$http.post("sys!createPublicTemplate.action",{template:$scope.newTemplate})
		.success(function(data){
			$msgDialog.showMessage(data.name);
			if (data.code==0) {
				$scope.$parent.goback();
...
.controller("templateModifyCtrl",function($scope,$http,$msgDialog,$selectDialog,$stateParams,$queryService){
	...
	var checkInput=function(){
		if (!$scope.template.template_name)return "请填写'模板名称'！";
		if ($scope.device_id==0)return "请选择'设备类型'！";
		if ($scope.part_id==0)return "请选择'部位类型'！";
		if (!$scope.template.pic_opinion)return "请填写'影像所见'！";
		if (!$scope.template.pic_conclusion)return "请填写'诊断意见'！";
	};
	$scope.queryMyTemplate=function(){
		$http.post("sys!selectPublicTemplateById.action",{templateId:$stateParams.templateId})
		.success(function(data){
			if (data.code==0) {
	...
	$scope.modifyTemplate=function(){
		...
		$http.post("sys!modifyPublicTemplate.action",{template:$scope.template})
		.success(function(data){
			$msgDialog.showMessage(data.name);
			if (data.code==0) {
				$scope.template=data.data.reportTemplate;
